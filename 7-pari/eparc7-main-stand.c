...
int main(int argc, char * argv [])
{
  ...
  char      s[512];
  long long high[9], low[9], llval, bigB;
  long      ltop, par1, par2;
  double    btmp, boundn;
  double    newval, lval;
  GEN		bigbgen;
  ...
  /* PAR HACK JJJJ */
					...
					*/
                    for(count[2] = par2; count[2] <= par2; count[2]++) {
                      A2 = count[2];
                      myaffsi( A2, coefs[2]);
...
/* Inherited from An bound */
  ...
  gettimeofday(&time2, &tzn);
  timetot = time2.tv_sec - time1.tv_sec;
  fprintf(logfile, "(%Ld-%ld-%ld",bigB,par1,par2);
  fprintf(logfile,"), %1ld polynomials, %1ld seconds, \
%3.2f T2, %1ld past disc, %1ld past T2, %1ld discf\n",
          polcount, timetot, Dmaxroot, postdisc, t2count, discfcount);
  fflush(logfile);
