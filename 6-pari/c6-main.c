                  ...
                  */
                  ...
                  do {
                    ...
                    low[2] = (long long) S2c_lower(- Dmaxroot, 2);
                     ...
                     /*
                      if(count[1]==3) {
                      printf(" dmax = %7.2f low|high = %lld|%lld; arc=%lld; shift=%lld,centers=%7.2f, a1=%lld\n",
                             Dmaxroot,low[2],high[2],arc[2],shift[2], centers[2],A1);
                    }
                     */
                    ...
                    for(count[2] = low[2]; count[2] <= high[2]; count[2]++) {
                      ...
                      if(verbosity>2) {
                        printf("\nLoop at (%1ld,%1ld) ---> (%1d,%lld), %1d range: %lld to %lld mults [%lld,%lld,%lld] (%lld)\n",
                               count[1], count[2],
                               (int) ((DEG-2*shift[1])/(2*arc[1])),
                          high[2], DEG,low[DEG],high[DEG],arc[1],arc[2],
                               arc[DEG],shift[DEG]);
                        fflush(stdout);
                      }
