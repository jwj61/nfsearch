...
int t41init(long pnum, long which, long what) {
  long j;
  ...
  */
  ...
  case 2: /* Capture b, loop through values of a */
    ...
    break;
  default:
    fprintf(stderr,"Error in t41 init.\n");
    break;
  }
  return 1;
}
...
/* increment counter number pnum modulo p
     returns 1 if ok */
long t41next(long pnum, long which) { /* This should never be called */
  long long a2, ab, a4;
  
  if(which != 2) {
    fprintf(stderr,"Error in t41 next.\n");
    return 0;
  }
  aa++;
  if(aa==pp)
  ...
  pdata[pnum][2][1] = truemod(4*bb*aa-10*a2, pp);
  pdata[pnum][3][1] = truemod(6*ab*aa-20*a2*aa, pp);
  pdata[pnum][4][1] = truemod(4*ab*a2-15*a4, pp);
  pdata[pnum][5][1] = truemod(bb*a4-4*aa*a4, pp);
  return 1;
}
